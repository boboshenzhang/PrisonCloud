...
public final class SecurityUtil {
    ...
    /**
   	 ...
   	 */
   	public static   Salt entryptPassword(String password) {
   		byte[] salt = Digests.generateSalt(SALT_SIZE); 
   		byte[] hashPassword = Digests.sha1(password, salt,
   				HASH_INTERATIONS);
   		return new Salt(Encodes.encodeHex(hashPassword),Encodes.encodeHex(salt));
   	}
   	/**
   	 * 设定安全的密码，生成随机的salt并经过1024次 sha-1 hash
   	 */
   	public static String entryptPassword(String password,String  salt) {
   		...
   		byte[] hashPassword = Digests.sha1(password.getBytes(), Encodes.decodeHex(salt),
   				HASH_INTERATIONS); 
   		return  Encodes.encodeHex(hashPassword);
   	}
   	
    public static void main(String[] args) throws Exception {
    	System.out.println(entryptPassword("adminzkss", "406dd29e9ccd51b4"));
        System.out.println(encryptDes("admin"));
